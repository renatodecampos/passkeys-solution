...
- **O que fazer**: Leia os campos `## Notas` de cada arquivo de status:
  - `tasks/fase-1-status.md`
  - `tasks/fase-1b-testes-server.md`
  - `tasks/fase-2-status.md`
  - `tasks/fase-3-status.md`
...
- **Status**: `[x] completed`
- **depends_on**: [4.1]
- **Arquivo**: `tasks/token-report.md` (criar)
- **O que fazer**:
  1. Leia a seção `## Token Usage` de cada arquivo de status:
     ...
     - `tasks/fase-2-status.md`
     - `tasks/fase-3-status.md`
     - `tasks/fase-4-documentacao.md` (preencha o valor desta fase antes de gerar)
  2. Crie `tasks/token-report.md` com a tabela consolidada abaixo
...
- **Formato do relatório**:
  ...
  **Ferramenta principal**: Cursor (Sonnet 4.6)
  ...
  - ...
  ```
- **Verificação**: arquivo `tasks/token-report.md` criado com todas as fases preenchidas
...
> Estas instruções são lidas automaticamente quando você executa `/feature-dev execute a fase 4`

**Pré-condição**: verifique que `tasks/fase-3-status.md` está `[x] completed`. Se não estiver, pare e informe.
...
- Crie `tasks/token-report.md` conforme spec da subtarefa 4.5
